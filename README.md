...
d.) Clean-Up. After you Navigate to the Stacks > Output > WebsiteURL field. This will contain the public URL of your EC2 instance.
Open a browser and go to the URL listed under WebsiteURL. You should see a simple message such as "Simple Budget Tracker, or "Hello from the ASG" Cleaning up requires you to select the stack created "MyStack" and click delete. confirmation wants "delete" typed in. > the stack changes to DE:ETE_IN_PROGRESS. > next navigate to s3 to delete the cf- bucket > choose empty > empty bucket page permanently delete. > choose exit > click the bucket name > choose delete > type the name > choose delete bucket. > Congratulations you succeeded creating a stack with numerous services!
