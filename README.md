...
d.) Clean-Up. Cleaning up requires you to select the stack created "MyStack" and click delete. > confirmation wants "delete" typed in. > the stack changes to DELETE_IN_PROGRESS. > next, navigate to s3 to delete the cf-template bucket > choose empty > empty bucket page by typing, permanently delete. > choose exit > click the bucket name > choose delete > type the name and choose delete bucket. > End of Project.
e.) What we did is a transition from a simple template to a high-availability setup, we’ll swap the single EC2 instance for an Auto Scaling Group (ASG) and an Application Load Balancer (ALB). We added a 
